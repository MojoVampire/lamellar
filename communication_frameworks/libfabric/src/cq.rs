...
use crate::{
    enums::{CompletionFlags, WaitObjType},
    error::Error,
    fid::{AsRawFid, AsRawTypedFid, CqRawFid, OwnedCqFid, RawFid},
    MappedAddress, RawMappedAddress,
};
...
/// A trait that provides the ability to wait on a completion queue configured as waitable.
pub trait WaitCq: AsTypedFid<CqRawFid> {
    ...
    fn sread_in(
        &self,
        count: usize,
        buffer: &mut Completion,
        cond: usize,
        timeout: i32,
    ) -> Result<(), crate::error::Error> {
        let p_cond = cond as *const usize as *const std::ffi::c_void;
        let err = read_cq_entry!(
            libfabric_sys::inlined_fi_cq_sread,
    ...
    fn sreadfrom_in(
        ...
        cond: usize,
        timeout: i32,
    ) -> Result<Option<MappedAddress>, crate::error::Error> {
        let p_cond = cond as *const usize as *const std::ffi::c_void;
        let mut address = 0;
        let p_address = &mut address;
        let err = read_cq_entry!(
            libfabric_sys::inlined_fi_cq_sreadfrom,
            self.as_typed_fid_mut().as_raw_typed_fid(),
            count,
            buffer,
            p_address,
        ...
        let address = if address == crate::FI_ADDR_NOTAVAIL {
            ...
            Some(MappedAddress::from_raw_addr_no_av(
                RawMappedAddress::Unspec(address),
            ))
        };
    ...
    fn signal(&self) -> Result<(), crate::error::Error> {
        let err = unsafe {
            libfabric_sys::inlined_fi_cq_signal(self.as_typed_fid_mut().as_raw_typed_fid())
        };
...
pub trait ReadCq: AsTypedFid<CqRawFid> + SyncSend {
    ...
    fn readfrom_in(
        ...
        let err = read_cq_entry!(
            libfabric_sys::inlined_fi_cq_readfrom,
            self.as_typed_fid_mut().as_raw_typed_fid(),
            count,
            buffer,
            p_address
    ...
    fn readerr_in(
        &self,
        err_buff: &mut CompletionError,
        flags: u64,
    ) -> Result<(), crate::error::Error> {
        let ret = {
            unsafe {
                libfabric_sys::inlined_fi_cq_readerr(
                    self.as_typed_fid_mut().as_raw_typed_fid(),
                    err_buff.get_mut(),
                    flags,
                )
    ...
    fn print_error(&self, err_entry: &crate::cq::CompletionError) {
        let ret = unsafe {
            libfabric_sys::inlined_fi_cq_strerror(
                self.as_typed_fid_mut().as_raw_typed_fid(),
...
impl<const WAIT: bool, const RETRIEVE: bool, const FD: bool> ReadCq
    for CompletionQueueImpl<WAIT, RETRIEVE, FD>
{
    fn read(&self, count: usize) -> Result<Completion, crate::error::Error> {
...
impl<'a, const WAIT: bool, const FD: bool> WaitObjectRetrieve<'a>
    for CompletionQueueImpl<WAIT, true, FD>
{
...
/// A trait that provides the ability to retrieve the underlying wait object of a libfabric object configured with a retrievable wait object.
    ...
    CompletionQueueImpl<WAIT, RETRIEVE, FD>
{
    pub(crate) fn new<EQ: ?Sized + 'static + SyncSend>(
        ...
        let err = unsafe {
            libfabric_sys::inlined_fi_cq_open(
        ...
        if err != 0 {
            ...
            Ok(Self {
                ...
                c_cq: OwnedCqFid::from(c_cq, domain.c_domain.domain.clone()),
                wait_obj: Some(attr.c_attr.wait_obj),

                _domain_rc: domain,
                entry_buff: if attr.c_attr.format == libfabric_sys::fi_cq_format_FI_CQ_FORMAT_UNSPEC
                {
...
impl<const WAIT: bool, const RETRIEVE: bool, const FD: bool>
    CompletionQueue<CompletionQueueImpl<WAIT, RETRIEVE, FD>>
{
    ...
    pub(crate) fn new<EQ: ?Sized + 'static + SyncSend>(
        ...
        Ok(Self {
            inner: MyRc::new(CompletionQueueImpl::new(
                domain,
                attr,
                c_void,
                default_buff_size,
...
impl<'a, T: WaitObjectRetrieve<'a>> CompletionQueue<T> {
    //[TODO] Make this a method of the trait ?
...
impl<'a, const WAIT: bool, const RETRIEVE: bool, const FD: bool>
    CompletionQueueBuilder<'a, WAIT, RETRIEVE, FD>
{
    ...
    ) -> CompletionQueueBuilder<'a, true, false, false> {
    ...
    ) -> Result<CompletionQueue<CompletionQueueImpl<WAIT, RETRIEVE, FD>>, crate::error::Error> {
        // CompletionQueue::new(self.options, self.domain, self.cq_attr, self.ctx, self.default_buff_size)
        CompletionQueue::<CompletionQueueImpl<WAIT, RETRIEVE, FD>>::new(
            domain.inner.clone(),
            self.cq_attr,
            self.ctx,
...
/// A single completion entry of type `Format`.
#[derive(Clone, Debug)]
    ...
    pub unsafe fn buffer<T>(&self) -> &[T] {
        unsafe {
            std::slice::from_raw_parts(
                self.c_entry.buf as *const T,
                self.c_entry.len / std::mem::size_of::<T>(),
            )
        }
    }
...
impl CompletionError {
    ...
    pub unsafe fn buffer<T>(&self) -> &[T] {
        unsafe {
            std::slice::from_raw_parts(
                self.c_err.buf as *const T,
                self.c_err.len / std::mem::size_of::<T>(),
            )
        }
    }
    ...
    fn cq_signal() {
        ...
        if let Err(ref err) = ret {
            if !(matches!(err.kind, crate::error::ErrorKind::TryAgain)
                || matches!(err.kind, crate::error::ErrorKind::Canceled))
            {
                ret.unwrap();
            }
        }
